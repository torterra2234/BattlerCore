...
func transition_from_warp(map : String, tile : Vector2) -> void:
	...
	print(tile)
	if map == "Overworld":
		transition_to_scene("assets/maps/house1.tscn",Vector2(1,2))
	else:
		transition_to_scene("assets/maps/overworld.tscn",Vector2(2,2))

func transition_to_scene(new_scene : String,location : Vector2) -> void:
